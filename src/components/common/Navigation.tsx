...
import { getNavItems, MOBILE_TABS, PRIMARY_CTA, ROLE_LABELS } from '../../config/navigation'
...
const NAV_ICONS: Record<PageId, typeof HomeIcon> = {
  dashboard: HomeIcon,
                ...
                return (
                  <li key={item.id}>
                    <button
                      ...
                      className={`focus-ring flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${
                        active
                          ? 'bg-[var(--cv-accent-soft)] font-semibold text-[var(--cv-accent)]'
        ...
        <div className="shrink-0 border-t border-[var(--cv-border)] p-3">
          ...
          {!sidebarCollapsed ? (
            <div className="mt-2 flex items-center gap-3 rounded-lg bg-[var(--cv-elevated)] px-3 py-2.5">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[var(--cv-btn-bg)] text-sm font-bold text-[var(--cv-accent)] ring-1 ring-[var(--cv-accent)]/30">
                {firstName.slice(0, 1)}
              </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {roles.length > 1 ? (
            <select
              ...
              className="focus-ring hidden max-w-[160px] appearance-none rounded-xl border border-[var(--cv-border)] bg-[var(--cv-elevated)] py-2 pl-3 pr-8 text-sm font-medium text-[var(--cv-text)] sm:block"
              value={role}
          ...
          <button
            className="flex h-9 w-9 items-center justify-center rounded-full bg-[var(--cv-btn-bg)] text-sm font-bold text-[var(--cv-accent)] ring-1 ring-[var(--cv-accent)]/30 md:hidden"
            type="button"
          ...
          return (
            <li key={tab}>
              <button
                ...
                style={{ color: active ? 'var(--cv-accent)' : 'var(--cv-muted)' }}
                type="button"
