    ...
    <main className="min-h-screen bg-[var(--cv-bg)] px-4 py-8">
      <div className="mx-auto w-full max-w-2xl">
        ...
        <form onSubmit={submit} className="rounded-xl border border-[var(--cv-border)] bg-[var(--cv-surface)] p-6 md:p-8">
          ...
          {step === 2 && (
            <div>
              <h1 className="text-3xl font-bold text-slate-900">What do you do?</h1>
              <p className="mt-2 text-slate-500">Select one or more roles that apply to you</p>
                ...
                {ALL_ROLES.map((role) => {
                  ...
                  return (
                    <button
                      ...
                      style={selected ? { borderColor: ROLE_COLORS[role], backgroundColor: `${ROLE_COLORS[role]}10` } : undefined}
                    >
              ...
              {errors.roles ? <p className="mt-3 text-sm text-red-600">{errors.roles}</p> : null}
            </div>
          )}
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {LANGS.map((lang) => (
                        <label key={lang} className="flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-sm">
                          <input
                            type="checkbox"
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-500">We&apos;ll use this for payouts</p>
          ...
          {step === 6 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-500">Clear photos required. Role-specific docs appear below.</p>
              {['Aadhaar Card', 'PAN Card'].map((doc) => (
                <div key={doc} className="rounded-lg border-2 border-dashed border-slate-300 p-4">
                  <p className="font-medium">{doc} <span className="text-red-500">*</span></p>
              ...
              {data.roles.includes('seller') && (
                <div className="rounded-lg border-2 border-dashed border-slate-300 p-4">
                  ...
                  <p className="text-xs text-slate-500">Clear photos, all 4 sides visible</p>
                  <input type="file" accept="image/*,.pdf" className="mt-2 block w-full text-sm" />
                </div>
              )}
              {data.roles.includes('buyer') && (
              ...
              {data.roles.includes('rental') && (
                <>
                  <div className="rounded-lg border-2 border-dashed border-slate-300 p-4">
                    <p className="font-medium">Equipment Ownership Proof <span className="text-red-500">*</span></p>
                    <input type="file" accept="image/*,.pdf" className="mt-2 block w-full text-sm" />
                  </div>
                  <div className="rounded-lg border-2 border-dashed border-slate-300 p-4">
              ...
              {(data.roles.includes('service') || data.roles.includes('educator')) && (
                <div className="rounded-lg border-2 border-dashed border-slate-300 p-4">
                  <p className="font-medium">Certifications / Qualifications <span className="text-red-500">*</span></p>
                  <input type="file" accept="image/*,.pdf" className="mt-2 block w-full text-sm" />
                </div>
              )}
            </div>
